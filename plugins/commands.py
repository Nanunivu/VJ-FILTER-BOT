...
async def start(client, message):
    await message.react(emoji="🤝", "😇", "🤗", "😍", "👍", "🎅", "😐", "🥰", "🤩", "😱", "🤣", "😘", "👏", "😛", "😈", "🎉", "⚡️", "🫡", "🤓", "😎", "🏆", "🔥", "🤭", "🌚", "🆒", "👻", "😁", "💘", "😨", "👀", "🎃", "💋", "🐳", "❤️‍🔥", "⚡️", "❤️")
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            InlineKeyboardButton('✨ Aᴅᴅ Mᴇ Tᴏ Yᴏᴜʀ Gʀᴏᴜᴘ ✨', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ],[
            InlineKeyboardButton('❤️ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ ❤️', url="https://t.me/shettyDiscus_Group"),
            InlineKeyboardButton('💥 Mᴏᴠɪᴇ Gʀᴏᴜᴘ 💥', url='https://t.me/shettymoviess')
        ],[
            InlineKeyboardButton('⚡️ Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ⚡️', url="https://t.me/shettymoviess")
        ]]
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('💥 Mᴏᴠɪᴇ Gʀᴏᴜᴘ 💥', url="https://t.me/+BSZtZ3uu20wwMzVl")
            ],[
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url=f'https://t.me/shettyDiscus_Group'),
                        InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url="https://t.me/shettymoviess")
                    ],[
                        InlineKeyboardButton('Rᴇǫᴜᴇsᴛ Gʀᴏᴜᴘ', url="https://t.me/+BSZtZ3uu20wwMzVl")
                    ],[
    ...
    if not files_:
        ...
        try:
            ...
            title = '@shettymoviess  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_
    title = '@shettymoviess  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files["file_name"].split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@shettymoviess  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files['file_name'].split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
